...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:4200", "http://localhost:53796"],  # Add both development ports
...
try:
    ...
    for collection in collections:
        ...
        # Log a sample document from each collection
        if count > 0:
            sample = db[collection].find_one()
            logger.info(f"Sample document from {collection}: {sample}")
