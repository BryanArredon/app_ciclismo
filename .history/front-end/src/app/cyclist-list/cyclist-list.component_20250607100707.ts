...
export class CyclistListComponent implements OnInit, OnDestroy {
  ...
  saveCyclist(): void {
    ...
    // Add fecha_registro only for new cyclists
    if (!this.isEditing) {
        cyclist.fecha_registro = new Date();
    }

    // Log the operation being performed
    console.log(`${this.isEditing ? 'Updating' : 'Creating'} cyclist:`, cyclist);
    if (this.isEditing) {
        console.log('With ID:', this.currentId);
    }
    ...
    const sub = request$.pipe(
        catchError((error: any) => {
            ...
            this.errorMessage = error.error?.detail || 'Error al procesar la operación. Por favor, intente nuevamente.';
            return throwError(() => error);
        }),
        finalize(() => {
